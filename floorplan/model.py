...
class PyramidPooling(nn.Module):
    def __init__(self, in_channels, channels=512, scales=(1, 2, 3, 6)):
        ...
        self.branches = nn.ModuleList(
            [
                nn.Sequential(
                    nn.AdaptiveAvgPool2d(scale), ConvBNReLU(in_channels, channels, 1)
                )
                for scale in scales
            ]
        )
    ...
    def forward(self, x):
        # Collect context at several spatial scales and align it with C4.
        pooled = [
            F.interpolate(
                branch(x),
                x.shape[-2:],
...
class UPerHead(nn.Module):
    def __init__(self, in_channels=(96, 192, 384, 768), channels=512, dropout=0.1):
        ...
        self.lateral = nn.ModuleList(
            [ConvBNReLU(value, channels, 1) for value in in_channels[:-1]]
        )
        self.fpn_convs = nn.ModuleList(
            [ConvBNReLU(channels, channels) for _ in in_channels[:-1]]
        )
        self.fpn_bottleneck = ConvBNReLU(len(in_channels) * channels, channels)
        self.classifier = nn.Sequential(
            nn.Dropout2d(dropout), nn.Conv2d(channels, 1, 1)
        )
    ...
    def forward(self, features, output_size):
        ...
        # Propagate semantic context from coarse to fine feature levels.
        ...
        outputs = [self.fpn_convs[index](laterals[index]) for index in range(3)] + [
            laterals[3]
        ]
        size = outputs[0].shape[-2:]

        # Fuse all FPN levels at the highest decoder resolution.
        outputs = [outputs[0]] + [
...
class HomographyHead(nn.Module):
    def __init__(self, in_channels=768):
        ...
        # Zero residuals make the initial transformation exactly identity.
    ...
    def forward(self, x):
        ...
        # Eight residual parameters define H while H[2, 2] fixes its scale.
        return torch.cat(
            [
                one + a[:, 0:1],
                a[:, 1:2],
...
def warp_homography(x, homography):
    ...
    # grid_sample expects destination pixels mapped back to source pixels.
    ...
    points = (
        torch.stack((xs, ys, torch.ones_like(xs)))
        .reshape(1, 3, -1)
        .expand(batch, -1, -1)
    )
    source = torch.linalg.solve(homography.float(), points)

    # Stabilize the homogeneous division near the projective horizon.
    z = source[:, 2:3]
    eps = torch.finfo(source.dtype).eps
    z = torch.where(z.abs() < eps, torch.where(z >= 0, eps, -eps), z)
    grid = (
        (source[:, :2] / z).transpose(1, 2).reshape(batch, height, width, 2).to(x.dtype)
    )
...
class WallProjectionNet(nn.Module):
    def __init__(self, in_channels=4):
        ...
        # Replace the RGB stem with a checkpoint-compatible RGBD stem.
        stem = nn.Conv2d(
            in_channels,
    ...
    def forward_backbone(self, x):
        # Return one feature map from every ConvNeXt resolution stage.
        x = self.features[0](x)
    ...
    def forward(self, x):
        ...
        # A single global homography projects the top mask onto the floor.
        return {
