...
COLORS = np.array([(61, 184, 255), (255, 99, 132), (88, 214, 141), (255, 194, 71),
                   (171, 112, 255), (255, 126, 69), (56, 220, 210), (235, 92, 220)], np.uint8)
...
class ExtractionResult:
    ...
    def save(self, folder):
        ...
        (folder / "rooms.json").write_text(json.dumps(self.data, indent=2), encoding="utf-8")
...
def letterbox(image, size=512):
    ...
    return np.asarray(canvas), (width, height, scale, *offset, resized.width, resized.height)
...
def heatmap(probability):
    colored = cv2.applyColorMap(np.uint8(np.clip(probability, 0, 1) * 255), cv2.COLORMAP_TURBO)
...
def annotate(rgb, rooms):
    ...
    for room in rooms:
        ...
        cv2.putText(image, text, center, cv2.FONT_HERSHEY_SIMPLEX, 0.55, (20, 20, 20), 3, cv2.LINE_AA)
        cv2.putText(image, text, center, cv2.FONT_HERSHEY_SIMPLEX, 0.55, (255, 255, 255), 1, cv2.LINE_AA)
    return image
...
class FloorPlanExtractor:
    def __init__(self, weights=None, device="auto", depth_model=None):
        ...
        state = torch.load(weights or default, map_location=self.device, weights_only=True)
        self.size = int(state.get("size", 512))
        self.model = WallProjectionNet(state.get("in_channels", 4)).to(self.device).eval()
        self.model.load_state_dict(state["model"])
        self.depth = DepthEstimator(self.device, depth_model) if depth_model else DepthEstimator(self.device)

    @torch.inference_mode()
    def extract(self, source, preset="weak"):
        if preset not in PRESETS:
            raise ValueError(f"Unknown preset: {preset}")
        image = Image.open(source).convert("RGB") if not isinstance(source, Image.Image) else source.convert("RGB")
        ...
        inputs = torch.cat(((rgb_tensor - MEAN) / STD, depth_tensor))[None].to(self.device)
        ...
        for region in regions:
            ...
            if area:
                rooms.append({"id": len(rooms) + 1, "polygon": polygon.tolist(), "area_pixels": round(area, 1)})
        ...
        barrier = restore(walls.astype(np.uint8), transform, cv2.INTER_NEAREST).astype(bool)
        data = {"image_size": [width, height], "preset": preset, "room_count": len(rooms), "rooms": rooms}
        images = {
            "annotated.png": annotate(original, rooms),
            "probability.png": heatmap(probability),
            "wall_barrier.png": np.repeat(barrier[..., None], 3, 2).astype(np.uint8) * 255,
        }
