...
PRESETS = {
    "weak": {"strong": 0.50, "weak": 0.10, "close_radius": 8, "dilate_radius": 2},
    "conservative": {"strong": 0.50, "weak": 0.18, "close_radius": 8, "dilate_radius": 1},
}
...
def endpoint_direction(skeleton, start, support):
    ...
    for _ in range(2 * support):
        y, x = current
        neighbors = [(yy, xx) for yy in range(max(0, y - 1), min(skeleton.shape[0], y + 2))
...
def bridge_gaps(walls, max_gap=50, angle=12, support=15):
    ...
    for i, (a, first) in enumerate(zip(points, directions)):
        ...
        for j in range(i + 1, len(points)):
            ...
            if 3 <= distance <= max_gap and first @ direction >= cosine and second @ -direction >= cosine:
...
def wall_barrier(probability, preset):
    ...
    probability[:, :3] = probability[:, -3:] = 0
    walls = binary_propagation(probability >= config["strong"], mask=probability >= config["weak"])
    close = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * config["close_radius"] + 1,) * 2)
    dilate = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * config["dilate_radius"] + 1,) * 2)
...
def enclosed_rooms(walls, minimum_ratio=0.001, minimum_radius=2.0):
    ...
    seed[:, [0, -1]] |= free[:, [0, -1]]
    enclosed = free & ~binary_propagation(seed, mask=free)
    count, labels, stats, _ = cv2.connectedComponentsWithStats(enclosed.astype(np.uint8), 8)
    ...
    for label in range(1, count):
        ...
        polygon = cv2.approxPolyDP(contour, max(1, 0.005 * cv2.arcLength(contour, True)), True)[:, 0]
